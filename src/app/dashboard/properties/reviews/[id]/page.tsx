...
import UnitReviews from '@/pages-components/Admin/Components/Property/UnitReviews'
  ...
  if (userRole === 'admin') {
    return (
      <section className="admin-section bg-gray-100 min-h-screen flex flex-col md:flex-row justify-start gap-2">
        ...
        />
        <UnitReviews unitID={id} isSidebarCollapsed={isSidebarCollapsed} />
      </section>
    )
  }
...
export default function PropertyReviewsPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={
      ...
      <PropertyReviewsContent params={params} />
    </Suspense>
  )
}
